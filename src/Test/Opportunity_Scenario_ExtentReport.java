...
public class Opportunity_Scenario_ExtentReport {
	...
	public void  Opportunity_Layouts () throws IOException {
		log = report.createTest("Opportunity_Layouts");
		objOpportunityLayouts = new OpportunityLayouts(driver); 
		try {
			objOpportunityLayouts.gotoOpportunity(); 
			// TODO Auto-generated catch block
			log.log(Status.PASS, "opportunity -Check Field availables in Opportunity Layouts-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, "opportunity -Check Field availables in Opportunity Layouts-Fail"+log.addScreenCaptureFromPath(captureScreen()));
	...
	public void Opportunity_Date_Field_Type() throws IOException {
		log = report.createTest("Opportunity_Date_Field_Type");
		objDataType = new DataType(driver); 
		try {
			objDataType.toGetFieldtype();

			log.log(Status.PASS, "Check Field Data Type-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, "Check Field Data Type-Fail"+log.addScreenCaptureFromPath(captureScreen()));
			Assert.assertTrue(false);
			e.printStackTrace();
		}
	}
	@Test (priority = 4 )//,enabled=false)
	public void AutopopulateAnnualRevenue() throws IOException  {
		log = report.createTest("AutopopulateAnnualRevenue");

		objAnnualRevenueAutopopulate = new AnnualRevenueAutopopulate(driver);
		try {
			objAnnualRevenueAutopopulate.AnnualRevenue_Autopopulate();
			log.log(Status.PASS, "Opportunity - Annual Revenue field should be autopopulate from Account-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, "Opportunity - Annual Revenue field should be autopopulate from Account-Fail"+log.addScreenCaptureFromPath(captureScreen()));
			Assert.assertTrue(false);
			e.printStackTrace();
		}
	...
	public void Qualification_NewTask()  throws Exception {
		log = report.createTest("Qualification_NewTask");
		objQualificationTask = new QualificationTask(driver);
		try {
			objQualificationTask.Qualification_Task();
			log.log(Status.PASS, "Qualification - Task has been created-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, "Qualification - Task has been created-Fail"+log.addScreenCaptureFromPath(captureScreen()));
			Assert.assertTrue(false);
			e.printStackTrace();
		}
	...
	public void AutoPopulate_Contact() throws Exception {
		log = report.createTest("AutoPopulate_Contact");
		objContactDetails = new ContactDetails(driver);
		try {
			objContactDetails.Contact_Details();
			log.log(Status.PASS, " Opportunity-autopopulate from Contact-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, " Opportunity-autopopulate from Contact-Fail"+log.addScreenCaptureFromPath(captureScreen()));
			Assert.assertTrue(false);
			e.printStackTrace();
		}
	...
	public void Quote_created_tab() throws Exception {
		log = report.createTest("Quote_created_tab");
		objQuote_created = new Quote_created(driver);
		try {
			objQuote_created.goto_Quotes_tab("5","5");
			log.log(Status.PASS, " Quote has been created-Pass"+log.addScreenCaptureFromPath(captureScreen()));
		} catch (Exception e) {
			log.log(Status.FAIL, " Quote has been created-Fail"+log.addScreenCaptureFromPath(captureScreen()));
			Assert.assertTrue(false);
			e.printStackTrace();
		}
